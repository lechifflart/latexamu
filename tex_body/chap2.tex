\chapter{ State of the art theory} \label{chap:theory}
...
\end{align}
where capital indices refer to nuclei and lowercase indices refer to electrons. $Z_I$, $M_I$ and $\RR_I$ are the atomic number, the mass and the position in real space of nucleus $I$, and $m_e,\rr_i$ are the electron mass and position in real space. In this thesis, unless explicitly specified, we will use the atomic units, that is $\hbar = m_e = e =\pi/\varepsilon_0 =1$.
...
\be
	\hat{H}_e = \hat{T_e} + \hat{V}_{ee} + \hat{V}_{ext}
\ee
...
\ee
They are analogous to Schrödinger equation for a particle evolving in a local effective potential $v_{\text{eff}}$, that we have yet to determine. Their solutions are the auxiliary system's eigenvalues $\epsilon_i$ and eigenvectors $\psi_i$. Using these eigenfunctions we can construct the electronic density as  :
\begin{equation}
...
For our purposes, it is not necessary to describe every single electron in the crystal. In fact, for the range of energies we are interested in, the electrons that can be optically excited are those that occupy the highest valence shells.
The core electrons of the first $s$ and $p$ shells are bound too strongly to the nuclei to be excited by a few electron-Volts incoming light. Hence, to simplify the system, we use pseudopotentials to avoid describing the core electrons. Pseudopotentials also solve the problem of the divergence of the Coulomb potential as $\rr \to 0$, which leads to rapid oscillations in the wave functions of the occupied orbitals.\cite{martin2020electronic}
\begin{figure}[h!tb]
	\vspace{0.5cm}
	\setcapindent{2em}
	\centering
...
We consider a system of N interacting electrons. We will consider the effect of nuclei motion in a later section. We start from the Hamiltonian for interacting electrons in second quantization:
\begin{align}
\begin{split}
 	\hat{H}_e &= \int dx_1 \ \hpsidag(x_1) h(\rr_1) \hpsi(x_1) + \frac{1}{2} \iint dx_1 \  dx_2 \ \hpsidag(x_1) \hpsidag(x_2) v(\rr_1,\rr_2) \hpsi(x_2)\hpsi(x_1) \\
		&= \hat{H}_0 + \hat{H}_{int}.
\end{split}
\end{align}
where $\hat{\psi}(x)$ is a field operator in Schrödinger representation, $h$ is the single-particle Hamiltonian for non-interacting particles evolving in an external potential $V_{ext}$ and $v(\rr_1,\rr_2) = e^2 (4\pi \left| \rr_1 - \rr_2 \right|)^{-1}$ is the Coulomb interaction. In the above equation, $x$ is the combined space and spin variables $x = (\rr, \sigma)$. For the most part of this thesis, the spin dependence of most quantities will be implicit.
%
...
\end{equation}
where we introduce the notation $1 \equiv (x_1,t_1)$. For the interaction Hamiltonian we have :
\begin{equation}
	\hat{H}_I'(t) = e^{i\hat{H} t} \hat{H}'(t) e^{-i\hat{H} t} = \int dx dx' \hpsidag(x,t^+) \Phi(x,x';t) \hpsi(x',t)
\end{equation}
where we use the notation $t^+$ for $t+\delta$ with $\delta \to 0^+$. We define a time-evolution operator in terms of this interaction Hamiltonian :
\begin{equation}
...
We will now sketch a derivation for the equations of motion of the single-particle Green's function. To do this, we will make explicit the time dependence of each term in Eq. \eqref{eq:GF}.
...
\end{equation}
and equivalently for $1^{++}$. $1^-$ and $1^{--}$ follow from a change of sign.
...
\end{equation}
After multiplying from the left by $\int d1 G_0(4,1)$ we obtain :
\begin{equation}
...
\begin{empheq}[box=\widefbox]{align*}
	...
	&W(1,2) = v(1,2) + \int d34v(1,3)\tilde{\chi}(3,4) W(4,2)
\end{empheq}
$\Gamma$ and $W$ also satisfy Dyson equations, just as $G$. This is an exact set of coupled equations, that is solved self-consistently. The complexity to solve this set of equation comes from the vertex function $\Gamma$, which is a function of three points. Approximating $\Gamma$ in the polarizability yields the \acrfull{RPA}. Approximating it in the self-energy gives the \acrfull{GWA}. We present these two in the following.

%
\subsection{The Random Phase Approximation} 
The \acrshort{RPA} consists in neglecting the vertex corrections in the formula of the reducible polarizability Eq. \eqref{eq:tilde_chi}. The vertex function reduces to 
\begin{equation}
...
However, $\tilde{\chi}_0$ will be useful when dealing with optical properties. Overall, the \acrshort{RPA} gives a simplified means to compute the response functions and screening of the system.

%
\subsection{The $GW$ approximation}
To calculate properties of the correlated N-electron system, the Hedin's equations are a great tool but are unsolvable in practice. One has to chose a starting approximation for the self-energy, which contains the summation to all orders in the interaction expansion of $G$. One way to approximate the self-energy is to consider $\frac{\delta \Sigma(12)}{\delta G(4,5)} = 0$ so that $\Gamma = \delta(1,2)\delta(1,3)$, just as for the \acrshort{RPA}, and we obtain :
\begin{equation} 
\gS_{xc}(1,2) = iG(1,2)W(1,2). \label{eq:GW}
\end{equation}
This is the so-called $GW$ approximation, which gives good results for weakly-correlated materials.\cite{aryasetiawan1998gw} It is the first-order in the expansion of the self-energy in terms of the interaction $W$. All higher order terms that are involved in electronic correlations are neglected. Part of those can be included by recomputing $G$ self-consistently with the Hedin's equations. However in practice, it is not guaranteed that it leads to better results. The $GW$ method is commonly used as a one-shot calculation starting from the DFT Kohn-Sham eigenvalues and wave functions. This is referred to as the G$_0$W$_0$ approximation. Instead one can iterate the Hedin's cycle in the \acrshort{GWA} but calculating only the change in the poles of $G$. This is referred to as the self-consistent eigenvalue $GW$, or evGW. We will use both in the body of this thesis.
...
With this form of the Green's function we can reformulate the Dyson equation from Eq. \eqref{eq:Dyson}. Just as the Green's function, the self-energy depends only on the time difference $\tau = t_1 - t_2$. We can then take the Fourier transform of Eq. \eqref{eq:GF_EOM_SE} :
\begin{equation}
...
\end{equation}
Each couple $(nn')$ corresponds to a pair $(vc)$ or $(cv)$ of an occupied and an empty state. We remark that the exciton energies replaced the difference of quasiparticle energies $\epsilon_c - \epsilon_v$ in the denominator, which are the quasiparticle transition energies. The screened interaction contributes to the attraction between the electron and the hole, lowering the transition energy below the gap. The Coulomb interaction includes the local field effects, which have a significant contribution for inhomogeneous systems. Overall, the exciton energy will lie below the minimal quasiparticle transition energy, which is the energy of the gap.
The difference between the transition energy and the quasiparticle gap is called the \textit{exciton binding energy}. Excitonic effects can be extremely important in the optical properties of semiconductors, as we will see in the next section.
%
...
To make the distinction between short-range, microscopic variations and long-range, macroscopic variations of the quantities of interest in a periodic and infinite crystal, it is easier to make use of the space Fourier transform and to work in reciprocal space. For a function $f$ of two space variables and one frequency (or one time difference), its space Fourier transform is :
\begin{equation}
...
\begin{equation}
	\rho_{v\kk c\kk+\qq}(\GG) = \bra{c\kk+\qq} e^{i(\qq+\GG)\cdot \rr} \ket{v\kk} \label{eq:gen_dipoles}
\end{equation}
where $v,c$ denote for valence and conduction states. These matrix elements describe the transition from a valence state at $\kk$ to a conduction state at $\kk+\qq$ mediated by an electric field with momentum $\qq$. At this point we note that, for momentum conservation, the vector $\qq$ which lies in the first \acrshort{BZ} has to be equal to the difference of two crystal momenta $\kk$. 
...
\begin{equation}
	\bar{\chi}_{00}(\qq \to 0,\omega) = 2 \lim_{\qq \to 0} \sum_\lambda \frac{\left| \sum_{vc\kk} \bar{A}_\lambda^{v\kk c\kk+\qq} \rho_{v\kk c\kk+\qq}\right|^2}{\omega - E_\lambda + i\eta} \label{eq:chi_BSE}
\end{equation}
...
\end{equation}
where it should be understood that the $00$ indices mean that we take the first component of the inverse, and not the inverse of the first component. We now define the \emph{macroscopic} dielectric function as :
\begin{equation}
...
\end{equation}
At the \acrshort{BSE} level, the dipole matrix elements are replaced by their linear combination with exciton eigenvectors. Using Eq. \eqref{eq:chi_BSE}, it gives :
\begin{equation}
...
\end{equation}
As one can see in the delta functions of the two versions, the peaks are not given by the same excitations. Hence the spectra will exhibit different features, whether we consider independent particles or we include the electron-hole interaction with the BSE. This is illustrated in Fig. \ref{fig:abs}.
\begin{figure}[h!t]
	\vspace{0.5cm}
	\setcapindent{2em}
	\centering
	\includegraphics[width=0.8\textwidth]{abs.pdf}
	\caption{Absorption spectrum of bulk \acrshort{hBN}, computed at the independent particle level (orange) and at  the\acrshort{BSE} level (blue) of theory, compared to the experiment (red dots) of Ref. \cite{artus2021ellipsometry}. The vertical line is at the supposed gap energy. The orange curve watch shifted to match it. The inclusion of excitonic effects makes a striking difference from the independent particle level and gives a much better agreement with experiment, where peaks appear below the gap energy. Note that the blue curve is calculated to match only the first peak and all curves are normalized.}
	\label{fig:abs}
\end{figure}
...
where excited electron and holes are relaxed at the band extrema after scattering with phonons, electrons or other relaxation processes. Hence we reach a quasi-equilibrium distribution of particles where some electrons
...
\end{equation}
These two expressions differ by the occupation functions of the electrons and holes, but also by the presence of the photon density of states $\mathcal{G}(\omega)$ in one and the photon density per unit energy $\mathcal{N}(\omega)$ in the other. The two are linked by the relation $ \int\mathcal{N}(\omega) d\omega = \int \bar{\mathcal{N}} \mathcal{G}(\omega) d\omega$ where $\bar{\mathcal{N}}$ is the average photon number. \\
...
\end{equation}
where $M_s$ is the mass of atom $s$. The square of the phonon frequencies are eigenvalues of the dynamical matrix with the following eigenvalue problem :
\begin{equation}
...
\end{equation}
In the last equality, the electron-phonon coupling matrix element $g$ is introduced. It is the probability that a phonon of branch index $\mu$ and momentum $\qq$ scatters a Kohn-Sham electron from a state $\ket{m\kk-\qq} = \ket{\psi_{m\kk-\qq}}$ into another state $\ket{n\kk}$. It is written in a simplified form as :
\begin{equation}
...
\begin{tcolorbox}[enhanced jigsaw,breakable,pad at break*=1mm,
	colback=cyanamu!5!white,colframe=blueamu!90!white,title= Summary of Chapter {\hypersetup{linkcolor=.}\ref{chap:theory}}]
%
Summary of Section \ref{sec:DFT}
	\begin{itemize}
		\item Density Functional Theory allows to compute the ground state density $n(\rr)$ and the equilibrium crystal structure of materials.
		\item The Kohn-Sham eigenvalues $\epsilon_i(\rr)$ obtained from \acrshort{DFT} give an estimation of the band structure.
		\item Along with the Kohn-Sham wavefunctions $\psi_i(\rr)$, these are the starting quantities for the more involved calculations presented in the following sections.
	\end{itemize}
%
Summary of Section \ref{sec:MBPT}
	  \begin{itemize}
		  \item The $GW$ approximation includes many-electron effects for weakly correlated systems, that are absent in \acrshort{DFT}.
		  \item We obtain from this the quasiparticle corrections that give more accurate electronic band structures.
		  \item We switch to a basis of bound electron-hole pair, the so-called excitons, better suited to describe optical processes.
		  \item We compute the exciton properties with the Bethe-Salpeter equation, from which we obtain exciton energies and eigenvectors.
	  \end{itemize}  
%
Summary of Section \ref{sec:optics}
	\begin{itemize}
		\item We simulate optical spectra with the linear response function obtained from \acrshort{MBPT}, including excitonic effects.
		\item The imaginary part of the dielectric function, calculated \textit{ab initio}, gives access to the optical absorption spectrum.
		\item We use a steady state approximation to obtain the luminescence spectrum from the absorption one.
	\end{itemize}  
%
Summary of Section \ref{sec:phonons}
	\begin{itemize}
		\item Phonons are quanta of vibrations of the lattice.
		\item They are quasiparticles with frequencies and eigenvectors that we obtain from \acrshort{DFPT}.
		\item The coupling between electrons and phonons enters an Hamiltonian in second quantization, in the form of matrix elements.
	\end{itemize}  
\end{tcolorbox}
\newpage
