...
\end{equation}
As one can see in the delta functions of the two versions, the peaks are not given by the same excitations. Hence the spectra will exhibit different features, whether we consider the RPA or the electron-hole interaction in the BSE. This is illustrated in Fig. \ref{fig:abs}.
\begin{figure}[h!t]
	\vspace{0.5cm}
	\setcapindent{2em}
	\centering
	\includegraphics[width=0.8\textwidth]{abs.pdf}
	\caption{Absorption spectrum of bulk \acrshort{hBN}, computed at the \acrshort{RPA} independent particle (orange) and \acrshort{BSE} (blue) level of theory compared to the experiment of Ref. \cite{artus2021ellipsometry}. The vertical line is at the supposed gap energy. The inclusion of excitonic effects makes a striking difference from the independent particle level and gives a much better agreement with experiment, where peaks appear below the gap energy. Note that the blue curve is calculated to match only the first peak and all curves are normalized.}
	\label{fig:abs}
\end{figure}
