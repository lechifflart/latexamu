...
Typically, the wavelengths are in the visible or ultraviolet range, from 180 nm to 1200 nm, while the crystal unit cells are of the order of a few nm. It is then a reasonable approximation to assume that the variation of the external electric field over the unit cell is negligible. 
Hence the screening matrix elements can be expanded and become the \textit{dipole} matrix elements. Finally we can write the imaginary part of the macroscopic dielectric function in terms of the dipoles (check Fulvio's formulas). This is the quantity we compute and we compare to experiments. As one can see in the denominators, the peaks are not given by the same excitations. Hence the spectra are quite different, whether we consider the RPA or the electron-hole interaction in the BSE. This is illustrated in the figure for absorption.
