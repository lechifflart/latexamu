...
\begin{figure}[h!tbp]
	...
	\setcapindent{2em}
	\centering
	\includegraphics[width=0.5\textwidth]{Sketch_PES.png}
...
In order to be able to compute the Green's function and the related useful quantities, we need to reformulate the problem using the non-interacting Green's function $G_0$. We start by separating the part which comes only from the one-particle operators in the equation of motion \eqref{eq:GF_EOM_SE} :
\begin{equation}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is not at the definitive location
...
To make the distinction between short-range, microscopic variations and long-range, macroscopic variations of the quantities of interest in a periodic and infinite crystal, it is best make the space Fourier transform and to work in reciprocal space. For a function $f$ of two space variables and one frequency (or time) variable, its space Fourier transform is :
\begin{equation}
...
\end{equation}
From now on, we set ourselves in the optical limit, that is to say $\qq \to 0$, as we consider a longitudinal external field that carries a only a very small momentum. Also, it is useful to make the separation between long-range and short-range terms in the Coulomb potential $v = v_0 + \bar{v}$. We have 
\begin{equation}
\begin{array}{l l}
	\bar{v}(\GG)=0 & \quad \text{for } \GG=0 \\
	\bar{v}(\GG)=v(\GG) & \quad \textrm{for } \GG \neq 0,
\end{array}
\end{equation}
and we can define the corresponding polarizability, called the \textit{proper} response function :
\begin{equation}
	\bar{\chi}(1,2) = \tilde{\chi}(1,2) + \int d34 \tilde{\chi}(1,3) \bar{v}(3,4) \bar{\chi}(4,2)
\end{equation}
Now if we use $\bar{v}$ instead of $v$ in the Bethe-Salpeter kernel $\Xi$, we can compute the two-particle propagator without the long-range component of the Coulomb interaction, which gives :
\begin{equation}
	\bar{L}(1,2,1',2') = \tilde{L}(1,2,1',2') + \int d34 \tilde{L}(1,3,1',3) \bar{v}(3,4) \bar{L}(4,2,4,2') \label{eq:Lbar}
\end{equation}
and we have the usual relationship :
\begin{equation}
	\bar{\chi}(1,2) = -i \bar{L}(1,2,1^+,2^+)
\end{equation}
Eq. \eqref{eq:Lbar} is the most commonly used equation for the calculation of optical spectra in the context of the \gls{BSE}.
...
To account properly for the electron-hole interaction, which is crucial to accurately simulate absorption spectra, we can compute the response function at the \gls{BSE} level The diagonalization of the two-particle Hamiltonian $H^{2p}$ (\eqref{eq:BSE_H2p}) with $\bar{v}$ gives the excitonic eigenvectors $\bar{A}_lambda$ (we will omit the bar for simplicity). Using this, the head of the response function matrix is :
\begin{equation}
...
Relation between $\bar{\chi}$ and $\epsilon_M$

\subsection{Optical absorption}
% limit of dielectric matrix for q to 0


In optics experiments, the external fields have wavelength that are far larger than the characteristic length of the unit cells of the crystals we simulate. 
Typically, the wavelengths are in the visible or ultraviolet range, from 180 nm to 1200 nm, while the crystal unit cells are of the order of a few nm. It is then a reasonable approximation to assume that the variation of the external electric field over the unit cell is negligible. This is called the \textit{dipole approximation}.
On top of that, we are interested in macroscopic quantities, so we consider the optical limit $\qq \to 0$. With all this, we can expand the screening matrix elements to first order in $\qq$, and the first non-zeo term is $\propto \left| i\qq\cdot\bra{c}\ket\rr-\rr'{v} \right|^2 = q^2\left| \mathbf{n}\cdot\bra{c}\rr\ket{v} \right|^2$ where $\mathbf{n}$ is the unit vector pointing in the direction of $\qq$. We then define the \textit{dipole matrix elements} as :
\begin{equation}
	d_{cv\kk} = \mathbf{n} \cdot \bra{c\kk} \rr \ket{v\kk}
\end{equation}

Finally we can write the imaginary part of the macroscopic dielectric function in terms of the dipoles. For independent particles, it writes :
\begin{equation}
	\epsilon_2(\omega) = \frac{8\pi^2 e^2}{V} \sum_{cv\kk} \left| d_{cv\kk} \right|^2 \delta(\omega - (\epsilon_{c\kk} - \epsilon_{v\kk}) )
\end{equation}
At the \gls{BSE} level, it gives :
\begin{equation}
	\epsilon_2(\omega) = \frac{8\pi^2 e^2}{V} \sum_\lambda \left| \sum_{cv\kk} A_\lambda^{cv\kk} d_{cv\kk} \right|^2 \delta(\omega - E_\lambda)
\end{equation}

This is the quantity we compute and we compare to experiments. As one can see in the denominators, the peaks are not given by the same excitations. Hence the spectra are quite different, whether we consider the RPA or the electron-hole interaction in the BSE. This is illustrated in the figure for absorption.
% where to talk about dark or bright excitons ? probably here



\subsection{Photoluminescence}
% what it is generally and
% van Roosbroeck-Shockley relation

\begin{itemize}
	\item screening matrix elements
	\item rigorous expression for $\chi^{RPA}$ and $\chi^{BSE}$
	\item dipole approx : field not varying over unit cell => dipole matrix elements
	\item macroscopic dielectric function (mention fact that longitudinal part enough)
	\item absorption spectra; comparison RPA and BSE
	\item PL : vRS relation
\end{itemize}
