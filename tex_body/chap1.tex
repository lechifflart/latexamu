...
\textcolor{red}{add more explanations, see Fulvio's thesis}
...
We use the functional derivative identity which is derived in appendix B :
\begin{equation}
...
\begin{equation}
	\int d3 G^{-1}(1,3) G(3,2) = \int d3 G(1,3)G^{-1}(3,2) = \delta(1,2)
\end{equation}
...
\end{equation}
The reducible polarizability would be the derivative of the density with respect to the perturbation $\Phi$
...
\end{equation}
We now have a set of coupled self-consistent equations, where the limit $\Phi \to 0$ can be taken. We will see in the next section how to write $G$ in a way that is easier to compute.
